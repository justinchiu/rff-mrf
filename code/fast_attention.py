...
def rff_attn(q, k, projection_matrix):
    ...
    log_phi_q = kernel_cons(
        q, projection_matrix, None, (0,), None, is_query=True, eps=0, normalize_data=True)
    log_phi_k = kernel_cons(
        k, projection_matrix, None, (0,), None, is_query=False, eps=0, normalize_data=True)
...
# debug
#print(jnp.isnan(projection_matrices).any())
#print(jnp.isnan(projection_matrices).nonzero())

"""
print(jnp.isnan(attn_dists).any())
print(jnp.isnan(attn_dists).nonzero())

projection_matrix = projection_matrices[16]

kernel_cons = nonnegative_softmax_kernel_feature_creator
log_phi_q = kernel_cons(
    qu, projection_matrix, None, (0,), None, is_query=True, eps=1e-8, normalize_data=False)
log_phi_k = kernel_cons(
    ku, projection_matrix, None, (0,), None, is_query=False, eps=1e-8, normalize_data=False)
log_pots_hat = log_phi_q[:,None,:] + log_phi_k[None,:,:]
log_pots = lmm(log_phi_q, log_phi_k)
adist = jnp.exp(log_pots - lse(log_pots, -1, keepdims=True))

print(jnp.isnan(log_phi_q).any())
print(jnp.isnan(log_phi_k).any())
print(jnp.isnan(log_phi_q).nonzero())
print(jnp.isnan(log_phi_k).nonzero())
"""
