...
\todo{Hypothesis: Prove the (insert name) parallelism can only be obtained
when parallel elimination orderings are possible?}
However, it may be still be possible to obtain speedups through parallelization. 

\subsection{Distributive law failure}
We first demonstrate that, in a very simple tree MRF,
the linearization trick does not result in the same speedups as the linear-chain MRF.
